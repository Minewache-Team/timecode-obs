...
TEST(HTTPTimeTest, ParseValidDate)
{
	...
	ASSERT_TRUE(
		http_time_parse_date("Sun, 01 Mar 2026 12:34:56 GMT", &sec));
...
TEST(HTTPTimeTest, ParseDateDifferentDay)
{
	...
	ASSERT_TRUE(
		http_time_parse_date("Thu, 29 Feb 2024 00:00:00 GMT", &sec));
...
TEST(HTTPTimeTest, ParseDateMidnight)
{
	int64_t sec;
	ASSERT_TRUE(
		http_time_parse_date("Wed, 01 Jan 2025 00:00:00 GMT", &sec));
...
TEST(HTTPTimeTest, ParseDateEndOfDay)
{
	int64_t sec;
	ASSERT_TRUE(
		http_time_parse_date("Wed, 01 Jan 2025 23:59:59 GMT", &sec));
...
TEST(HTTPTimeTest, ParseInvalidMonth)
{
	int64_t sec;
	EXPECT_FALSE(
		http_time_parse_date("Sun, 01 Xyz 2026 12:34:56 GMT", &sec));
}
...
TEST(HTTPTimeTest, ParseInvalidDay)
{
	int64_t sec;
	EXPECT_FALSE(
		http_time_parse_date("Sun, 00 Mar 2026 12:34:56 GMT", &sec));
	EXPECT_FALSE(
		http_time_parse_date("Sun, 32 Mar 2026 12:34:56 GMT", &sec));
}
...
TEST(HTTPTimeTest, ParseInvalidYear)
{
	int64_t sec;
	EXPECT_FALSE(
		http_time_parse_date("Sun, 01 Mar 1969 12:34:56 GMT", &sec));
}
...
TEST(HTTPTimeTest, ParseAllMonths)
{
	...
	const char *dates[] = {
		...
		"Sat, 15 Feb 2025 12:00:00 GMT",
		"Sat, 15 Mar 2025 12:00:00 GMT",
		"Tue, 15 Apr 2025 12:00:00 GMT",
		"Thu, 15 May 2025 12:00:00 GMT",
		"Sun, 15 Jun 2025 12:00:00 GMT",
		"Tue, 15 Jul 2025 12:00:00 GMT",
		"Fri, 15 Aug 2025 12:00:00 GMT",
	...
	for (int i = 0; i < 12; i++) {
		EXPECT_TRUE(http_time_parse_date(dates[i], &sec))
			<< "Failed for month " << (i + 1);
	}
}
