...
static void make_sidecar_path(char *path, size_t pathsz)
{
	...
	if (dot)
		...
		snprintf(path + strlen(path), pathsz - strlen(path), ".ltc.json");
}

static void write_sidecar_end(const char *recording_path, struct metadata_writer *mw)
{
	...
	if (have_diag) {
		...
		const char *m = (diag.sync_method == SYNC_METHOD_NTP)    ? "NTP"
				: (diag.sync_method == SYNC_METHOD_HTTP) ? "HTTP"
									 : "local";
	...
	if (!f) {
		obs_log(LOG_WARNING, "Failed to update sidecar: %s", sidecar_path);
		return;
	}
	...
	if (start_utc)
		strftime(start_iso, sizeof(start_iso), "%Y-%m-%dT%H:%M:%SZ", start_utc);
	else
	...
	fclose(f);
	obs_log(LOG_INFO, "Metadata sidecar completed: %s (%.0fs)", sidecar_path, duration);
}
...
static void on_recording_event(enum obs_frontend_event event, void *data)
{
	...
	if (event == OBS_FRONTEND_EVENT_RECORDING_STARTED) {
		...
		snprintf(mw->start_timecode, sizeof(mw->start_timecode), "%s", mw->current_timecode);
...
metadata_writer_t *metadata_writer_create(void)
{
	...
	snprintf(mw->current_timecode, sizeof(mw->current_timecode), "00:00:00:00");
	snprintf(mw->start_timecode, sizeof(mw->start_timecode), "00:00:00:00");
			      ...
			      bool ntp_synced, int64_t ntp_offset_ms, const char *sync_method_str)
{
	...
	if (framerate_str)
		snprintf(mw->framerate_str, sizeof(mw->framerate_str), "%s", framerate_str);
	if (ntp_server)
		snprintf(mw->ntp_server, sizeof(mw->ntp_server), "%s", ntp_server);
	mw->ntp_synced = ntp_synced;
	mw->ntp_offset_ms = ntp_offset_ms;
	if (sync_method_str)
		snprintf(mw->sync_method, sizeof(mw->sync_method), "%s", sync_method_str);
}
...
void metadata_writer_set_timecode(metadata_writer_t *mw, const char *tc_str)
{
	if (!mw || !tc_str)
		return;
	snprintf(mw->current_timecode, sizeof(mw->current_timecode), "%s", tc_str);
}
